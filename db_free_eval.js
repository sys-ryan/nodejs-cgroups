...
if (process.argv.length !== 5) {
  console.log(
    `node ${path.basename(__filename)} [cgroupName] [processName] [period]`
  );
  process.exit();
}
...
processUtil.spawnChildProcess(pName).then((childProcess) => {
...
function setCallback() {
  ...
  return function cb() {
    ...
    // memory_limit = parseInt(memory_usage + 1024 * 16);
    // util.setMemoryLimit(cgName, memory_limit);

    data = util.getCgroupInfo(cgName);

    addData(
      "./evaluation_data/app-limit/0818_db_limit_100_x16.txt",
      `${data.memory_usage}:${data.memory_limit}:${data.tasks}\n`
    );
    console.log(
      `${cnt}:${data.memory_usage}:${data.memory_limit}:${data.tasks}\n`
    );
